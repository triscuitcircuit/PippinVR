...
- [[https://brew.sh/][MacOS Homebrew]]
- [[https://sidequestvr.com/setup-howto][SideQuest]] (V 1.21)
- Meta VR Headset (Quest 2, Quest 3, Quest 3s) in Developer Mode. (tested on the Meta Quest 3).
- [[https://formulae.brew.sh/formula/make#default][Make]]
- CMAKE ( [[https://cmake.org/][CMAKE.org]] or by [[https://formulae.brew.sh/formula/cmake][Homebrew]])
- [[https://formulae.brew.sh/cask/android-platform-tools#default][Android Platform Tools]]
- Xcode Command line tools (~xcode-select --install~)
- [[https://developer.apple.com/swift/][Swift]] (V.60)
- [[https://formulae.brew.sh/formula/gradle][Gradle]]
...
#+CAPTION: Diagram representing PippinVR-server flow. ~PipelineSession~ handles information from the ~VideoEncoder~, ~ScreenCapture~, ~FrameSink~, and ~VirtualDisplay~.
[[file:figures/UML_DisplayCapture.png]]
...
#+CAPTION: Actor diagram for lifetimes of the client. Showcases the loop of frame encoding and sending to the headset to ~FrameSink~
[[file:figures/UML_Lifetimes.png]]
...
* Contributing
Contributions are welcome and encouraged (as per the GNUv3 License). To contribute, make sure to use the ~.pre-commit~ file. To do so, make sure to install via [[https://formulae.brew.sh/formula/pre-commit][Homebrew]].
#+begin_src sh
  pre-commit install
#+end_src
