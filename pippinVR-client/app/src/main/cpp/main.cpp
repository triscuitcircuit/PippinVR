...
std::string systemProperty(const char* key, const char* fallback) {
    ...
    if (length > 0) return std::string(value, static_cast<size_t>(length));
...
void android_main(android_app* app) {
    ...
    const uint16_t port = static_cast<uint16_t>(
        std::atoi(systemProperty("debug.pippinvr.port", "9943").c_str()));
    LOGI("server target %s:%u (override with `adb shell setprop debug.pippinvr.host ...`)",
         host.c_str(), port == 0 ? kDefaultPort : port);

    state.client = std::make_unique<pippinvr::StreamClient>(
        host, port == 0 ? kDefaultPort : port);
    state.client->start(
        [&state](const std::vector<pippinvr::StreamInfo>& streams) {
            state.xr.onStreams(streams);
        },
        [&state](pippinvr::FramePacket&& packet) {
            state.xr.onFrame(std::move(packet));
        });
